...
public static class TechStackEndpoints
{
    ...
    {
        routes.MapGet("/api/techs", async (APIServices service) =>
        {
        ...
        routes.MapPut("/api/updatetech{id}", async (APIServices service, TechStack updatedTech, string id) =>
        {
            var storedTech = await service.GetByIdAsync(id);
            if (storedTech is null)
            {
        ...
        routes.MapPost("/api/newtech", async (APIServices service, TechStack newTech) =>
        {
            await service.CreateAsync(newTech);
            return Results.Ok(newTech);
        });

        routes.MapDelete("/api/deletetech{id}", async (APIServices service, string id) =>
        {
            var tech = await service.GetByIdAsync(id);
            if (tech is null)
            {
